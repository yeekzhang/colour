...
__all__ = [
    ...
    'function_gamma_BT2100_HLG', 'oetf_BT2100_HLG', 'oetf_reverse_BT2100_HLG',
    'eotf_BT2100_HLG', 'ootf_BT2100_HLG', 'ootf_reverse_BT2100_HLG'
]
...
def ootf_BT2100_HLG(E, L_B=0.005, L_W=1000, gamma=None):
    ...
    Y_S = np.sum(np.array([0.2627, 0.6780, 0.0593]) * E)

    if gamma is None:
        gamma = function_gamma_BT2100_HLG(L_W)

    F_D = alpha * Y_S ** (gamma - 1) * E + beta
...
def ootf_reverse_BT2100_HLG(F_D, L_B=0.005, L_W=1000, gamma=None):
    ...
    Y_S = np.sum(np.array([0.2627, 0.6780, 0.0593]) * F_D)

    if gamma is None:
        gamma = function_gamma_BT2100_HLG(L_W)

    E = (F_D - beta) / (alpha * Y_S ** (gamma - 1))
